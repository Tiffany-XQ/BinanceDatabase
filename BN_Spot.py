...
ROOT_PATH = os.getcwd()
SPOT_PATH = os.path.join(ROOT_PATH, 'Database', EXCHANGE.id, 'Spot')

def save_klines(params, date, symbol, time_interval):
    
    df = EXCHANGE.publicGetKlines(params=params)
    ...
    df.drop(["Unused"], axis=1)

    path = os.path.join(SPOT_PATH, str(pd.Timestamp(date).date()))
...
def main(date):
    ...
    for symbol in Symbols:
        ...
        for time_interval in Intervals:
            ...
            logging.info(f'Fetching Binance {pd.Timestamp(date).date()} {symbol} {time_interval} spot klines...')
