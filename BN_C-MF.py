...
ROOT_PATH = os.getcwd()
CMF_PATH = os.path.join(ROOT_PATH, 'Database', EXCHANGE.id, 'C-MF')

def save_klines(params, date, symbol, time_interval):
    
    df = EXCHANGE.dapiPublicGetKlines(params=params)    # Coin-M feature: dapi
    df = pd.DataFrame(df, dtype=float,
                    columns=['Open_time', 'Open', 'High', 'Low', 'Close', 'Volume',
                            'Close_time', 'Base_asset_vol', 'Num_trades',
                            'Taker_buy_vol', 'Taker_buy_base_asset_vol', 
                            'Ignore'])
    df['Open_time'] = pd.to_datetime(df['Open_time'], unit='ms')
    df['Close_time'] = pd.to_datetime(df['Close_time'], unit="ms")
    df.drop(["Ignore"], axis=1)

    path = os.path.join(CMF_PATH, str(pd.Timestamp(date).date()))
    os.makedirs(path, exist_ok=True)
    file_name = '_'.join([symbol, time_interval]) + '.csv'
...
def main(date):
    ...
    Symbols = ['BTCUSD_PERP', 'BTCUSD_221230', 'BTCUSD_230331', 'ETHUSD_PERP', 'ETHUSD_221230', 'ETHUSD_230331']
    ...
    for symbol in Symbols:
        ...
        for time_interval in Intervals:
            ...
            logging.info(f'Fetching Binance {pd.Timestamp(date).date()} {symbol} {time_interval} COIN-M Features klines...')
